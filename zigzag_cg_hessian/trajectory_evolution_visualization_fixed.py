#!/usr/bin/env python3
"""
Trajectory Evolution Visualization (Fixed)
...
class TrajectoryEvolutionVisualizer:
    ...
    def create_pca_from_trajectories(self, trajectories):
        ...
        print("Creating PCA model from trajectory data...")
    ...
    def plot_trajectory_evolution(self, trajectories, pca_model, save_dir='./zigzag_cg_hessian'):
        """Plot trajectory evolution showing path from start to end"""
        os.makedirs(save_dir, exist_ok=True)
        
        # Project trajectories to PCA space
        ddpm_pca = pca_model.transform(trajectories['DDPM']['xt'])
        hessian_pca = pca_model.transform(trajectories['Hessian_Free']['xt'])
    ...
    def create_trajectory_explanation_diagram(self, save_dir='./zigzag_cg_hessian'):
...
def main():
    ...
    print("🚀 Trajectory Evolution Visualization (Fixed)")
    print("="*60)
    ...
    try:
        ...
        print("Creating PCA model from trajectory data...")
        print(f"{'='*40}")
        pca_model = visualizer.create_pca_from_trajectories(trajectories)
        
        # Create trajectory evolution plots
        print(f"\n{'='*40}")
        print("Creating trajectory evolution visualization...")
        print(f"{'='*40}")
        visualizer.plot_trajectory_evolution(trajectories, pca_model)
