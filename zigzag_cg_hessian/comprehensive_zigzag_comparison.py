#!/usr/bin/env python3
"""
Comprehensive Zigzag Comparison: DDIM vs DDPM vs Hessian-Enhanced Methods
This script compares zigzag patterns across different sampling methods with large sample sizes
"""
...
class ZigzagAnalyzer:
    """Comprehensive zigzag pattern analyzer for different sampling methods"""
    ...
    def load_pipeline(self, method_name):
        ...
        if method_name == 'DDIM':
        ...
        elif method_name == 'DPM_LM_Original':
        ...
        elif method_name == 'DPM_LM_HessianFree':
            pipe.scheduler = DPMSolverMultistepHCGScheduler.from_config(pipe.scheduler.config)
            pipe.scheduler.config.solver_order = 3
            pipe.scheduler.config.algorithm_type = "dpmsolver"
            pipe.scheduler.lamb = 0.0008
            ...
            pipe.scheduler.set_model(pipe.unet)
        elif method_name == 'DPM_LM_Explicit':
            pipe.scheduler = DPMSolverMultistepHCGScheduler.from_config(pipe.scheduler.config)
            pipe.scheduler.config.solver_order = 3
            pipe.scheduler.config.algorithm_type = "dpmsolver"
            pipe.scheduler.lamb = 0.0008
            pipe.scheduler.lm = True
            pipe.scheduler.kappa = 1e-8
            pipe.scheduler.hessian_method = 'explicit'
            pipe.scheduler.set_model(pipe.unet)
    ...
    def _generate_single_trajectory(self, pipe, method_name, seed):
        ...
        for j, t in enumerate(scheduler.timesteps):
            # Store current state
            xt_flat = latents.view(1, -1).cpu().numpy().flatten()
            ...
            noise_pred_flat = noise_pred.view(1, -1).cpu().numpy().flatten()
    ...
    def plot_comprehensive_comparison(self, results_dict, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        ...
        fig.suptitle('Comprehensive Zigzag Analysis: DDIM vs DDPM vs Hessian-Enhanced Methods',
        ...
        save_path = os.path.join(save_dir, f'comprehensive_zigzag_comparison_{timestamp}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"\n✓ Comprehensive comparison plot saved to: {save_path}")

        plt.show()

    def generate_summary_table(self, results_dict, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        """Generate summary table of all methods"""
        os.makedirs(save_dir, exist_ok=True)

        print("\n" + "="*80)
        print("COMPREHENSIVE ZIGZAG ANALYSIS SUMMARY")
        print("="*80)
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        summary_path = os.path.join(save_dir, f'zigzag_analysis_summary_{timestamp}.txt')

        with open(summary_path, 'w') as f:
            f.write("COMPREHENSIVE ZIGZAG ANALYSIS SUMMARY\n")
            f.write("="*80 + "\n\n")
...
def main():
    """Main function to run comprehensive zigzag comparison"""

    print("🚀 Comprehensive Zigzag Analysis: DDIM vs DDPM vs Hessian-Enhanced Methods")
    print("="*80)
    print("Large-scale analysis with 5000+ samples for robust statistical comparison")
    print("="*80)

    # Initialize analyzer
    analyzer = ZigzagAnalyzer(n_samples=5000, num_inference_steps=25)
    ...
    try:
        ...
        print(f"{'='*60}")
        analyzer.plot_comprehensive_comparison(results_dict)

        # Generate summary table
        analyzer.generate_summary_table(results_dict)
