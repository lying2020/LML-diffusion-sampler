#!/usr/bin/env python3
"""
DDIM vs Hessian-Free: Anisotropy Analysis using Max and Median Eigenvalues
Analyzing the transition from isotropic to anisotropic during sampling process
"""
...
class AnisotropyAnalyzer:
    ...
    def analyze_anisotropy_patterns(self, trajectories, method_name, pca_model):
        ...
        if anisotropy_ratios:
            ...
            std_anisotropy = np.std(anisotropy_ratios)
            min_anisotropy = np.min(anisotropy_ratios)
    ...
    def plot_anisotropy_comparison(self, results_dict, eigenvalues, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        ...
        for i, method in enumerate(methods):
            ...
            ax1.set_xlabel(f'Max Eigenvalue PC ({result["explained_variance_ratio"][0]:.1%})')
            ax1.set_ylabel(f'Median Eigenvalue PC ({result["explained_variance_ratio"][1]:.1%})')
            ax1.set_title(f'{method}\nAnisotropy Trajectory\nAvg Anisotropy: {result["avg_anisotropy"]:.2f}')
            ...
            ax2.set_xlabel(f'Max Eigenvalue PC ({result["explained_variance_ratio"][0]:.1%})')
            ax2.set_ylabel(f'Median Eigenvalue PC ({result["explained_variance_ratio"][1]:.1%})')
            ...
            if anisotropy_ratios:
                ...
                ax3.set_ylabel('Anisotropy Ratio (Max/Median)')
        ...
        save_path = os.path.join(save_dir, f'ddim_vs__anisotropy_{timestamp}.png')
    ...
    def generate_anisotropy_report(self, results_dict, eigenvalues, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"Max eigenvalue: {eigenvalues[0]:.6f}\n")
            f.write(f"Median eigenvalue: {eigenvalues[len(eigenvalues)//2]:.6f}\n")
            ...
            for method, result in results_dict.items():
                ...
                f.write(f"  - Average anisotropy: {result['avg_anisotropy']:.3f}\n")
                f.write(f"  - Anisotropy std: {result['std_anisotropy']:.3f}\n")
                f.write(f"  - Min anisotropy: {result['min_anisotropy']:.3f}\n")
            ...
            f.write(f"- The transition from isotropic to anisotropic during sampling\n")
        ...
        for method, result in results_dict.items():
            ...
            print(f"  - Average anisotropy: {result['avg_anisotropy']:.3f}")
