...
def create_algorithm_flow_diagram():
    ...
    ax.text(1.5, 10, 'Input\n(noise_pred, x, t)', ha='center', va='center', fontsize=16, fontweight='bold')

    # 2. LML correction selection
    lml_box = FancyBboxPatch((3.25, 9.5), 3.5, 1, boxstyle="round,pad=0.1",
                             facecolor='lightgreen', edgecolor='green', linewidth=2)
    ax.add_patch(lml_box)
    ax.text(5.0, 10, 'Sampling Correction Method Selection', ha='center', va='center', fontsize=16, fontweight='bold')

    # 3. Three methods
    # Original LML
    orig_box = FancyBboxPatch((0.5, 7.5), 2.0, 1, boxstyle="round,pad=0.1",
                              facecolor='lightyellow', edgecolor='orange', linewidth=2)
    ax.add_patch(orig_box)
    ax.text(1.50, 8, 'LML\n(Simplified Approximation)', ha='center', va='center', fontsize=16)

    # Explicit Hessian
    expl_box = FancyBboxPatch((3.75, 7.5), 2.5, 1, boxstyle="round,pad=0.1",
                              facecolor='lightcoral', edgecolor='red', linewidth=2)
    ax.add_patch(expl_box)
    ax.text(5.0, 8, 'Explicit Hessian\n(Direct Computation)', ha='center', va='center', fontsize=16)

    # Hessian-Free
    hf_box = FancyBboxPatch((7.25, 7.5), 2.0, 1, boxstyle="round,pad=0.1",
                            facecolor='lightpink', edgecolor='purple', linewidth=2)
    ax.add_patch(hf_box)
    ax.text(8.25, 8, 'Hessian-Free\n(CG + HVP)', ha='center', va='center', fontsize=16)

    # 4. Hessian-Free detailed flow
    # HVP calculation
    hvp_box = FancyBboxPatch((7.25, 5.75), 2.0, 1, boxstyle="round,pad=0.1",
                             facecolor='lightcyan', edgecolor='cyan', linewidth=2)
    ax.add_patch(hvp_box)
    ax.text(8.25, 6.25, 'Hessian-Vector Product\n(Pearlmutter Method)', ha='center', va='center', fontsize=16)

    # CG algorithm
    cg_box = FancyBboxPatch((7.25, 4), 2.0, 1, boxstyle="round,pad=0.1",
                            facecolor='lightsteelblue', edgecolor='navy', linewidth=2)
    ax.add_patch(cg_box)
    ax.text(8.25, 4.50, 'Conjugate Gradient\n(Solve Hx = b)', ha='center', va='center', fontsize=16)

    # 5. Output
    output_box = FancyBboxPatch((3.5, 2), 3, 1, boxstyle="round,pad=0.1",
                                facecolor='lightgray', edgecolor='black', linewidth=2)
    ax.add_patch(output_box)
    ax.text(5, 2.5, 'Corrected Noise Prediction\n(corrected_noise)', ha='center', va='center', fontsize=16, fontweight='bold')

    # Connection lines
    # Input to Sampling Correction selection
    ax.arrow(2.5, 10.0, 0.75, 0, head_width=0.1, head_length=0.1, fc='black', ec='black')

    # Sampling Correction selection to three methods
    ax.arrow(4, 9.5, -1.50, -1, head_width=0.1, head_length=0.1, fc='black', ec='black')
    ...
    ax.arrow(8.25, 7.5, 0, -0.75, head_width=0.1, head_length=0.1, fc='purple', ec='purple', linewidth=2)
    ax.arrow(8.25, 5.75, 0, -0.75, head_width=0.1, head_length=0.1, fc='purple', ec='purple', linewidth=2)

    # All methods to output
    ax.arrow(1.75, 7.5, 1.75, -4.5, head_width=0.1, head_length=0.1, fc='orange', ec='orange')
    ax.arrow(5.0, 7.5, 0.0, -4.5, head_width=0.1, head_length=0.1, fc='red', ec='red')
    ax.arrow(8.25, 4, -1.65, -1.05, head_width=0.1, head_length=0.1, fc='purple', ec='purple', linewidth=2)

    # Add mathematical formulas
    ax.text(2.5, 5.0, 'H⁻¹ ≈ (I + λI)⁻¹', ha='center', va='center', fontsize=16,
            bbox=dict(boxstyle="round,pad=0.2", facecolor='white', edgecolor='orange'))

    ax.text(5.0, 5.0, 'H = ∇²f(x)\nUsing Finite Differences', ha='center', va='center', fontsize=16,
            bbox=dict(boxstyle="round,pad=0.2", facecolor='white', edgecolor='red'))

    ax.text(7.5, 3, 'Hv = ∇(∇f · v)\nCG: Hx = b', ha='center', va='center', fontsize=16,
            bbox=dict(boxstyle="round,pad=0.2", facecolor='white', edgecolor='purple'))

    # Add complexity information
    ax.text(0.5, 1, 'Complexity Comparison:', fontsize=16, fontweight='bold')
...
def create_cg_algorithm_diagram():
    ...
    # Initialization
    init_box = FancyBboxPatch((1, 5.8), 2, 1.2, boxstyle="round,pad=0.1",
                              facecolor='lightblue', edgecolor='blue', linewidth=2)
    ax.add_patch(init_box)
    ax.text(2, 6.4, 'Initialize\nx₀ = 0\nr₀ = b\np₀ = r₀', ha='center', va='center', fontsize=16)

    # Iteration loop
    loop_box = FancyBboxPatch((4, 5), 4, 2.2, boxstyle="round,pad=0.1",
                              facecolor='lightyellow', edgecolor='orange', linewidth=2)
    ax.add_patch(loop_box)
    ax.text(6, 7.0, 'CG Iteration Loop', ha='center', va='center', fontsize=16, fontweight='bold')
    ...
    # Add mathematical formulas
    ax.text(5, 1.5, 'Key Formulas:', fontsize=16, fontweight='bold')
...
def create_pearlmutter_method_diagram():
    ...
    step1_box = FancyBboxPatch((0.5, 4), 3.3, 1, boxstyle="round,pad=0.1",
                               facecolor='lightblue', edgecolor='blue', linewidth=2)
    ax.add_patch(step1_box)
    ax.text(2, 4.5, 'Step 1: Compute Gradient\ng = ∇f(x)', ha='center', va='center', fontsize=16)

    # Step 2: Compute dot product
    step2_box = FancyBboxPatch((4.5, 4), 3.3, 1, boxstyle="round,pad=0.1",
                               facecolor='lightgreen', edgecolor='green', linewidth=2)
    ax.add_patch(step2_box)
    ax.text(6, 4.5, 'Step 2: Compute Dot Product\ng·v', ha='center', va='center', fontsize=16)

    # Step 3: Compute second-order gradient
    step3_box = FancyBboxPatch((8.5, 4), 3.3, 1, boxstyle="round,pad=0.1",
                               facecolor='lightcoral', edgecolor='red', linewidth=2)
    ax.add_patch(step3_box)
    ax.text(10, 4.5, 'Step 3: Second-order Gradient\nHv = ∇(g·v)', ha='center', va='center', fontsize=16)

    # Step 4: Output
    step4_box = FancyBboxPatch((12.5, 4), 1.2, 1, boxstyle="round,pad=0.1",
                               facecolor='lightyellow', edgecolor='orange', linewidth=2)
    ax.add_patch(step4_box)
    ax.text(13, 4.5, 'Hv', ha='center', va='center', fontsize=16, fontweight='bold')
    ...
    ax.text(1.0, 2.5, 'Code Implementation:', fontsize=16, fontweight='bold')
    ax.text(1.0, 2, 'grad = torch.autograd.grad(log_prob, x_grad, create_graph=True)[0]',
            fontsize=14, fontfamily='monospace')
    ax.text(1.0, 1.7, 'grad_dot_v = torch.sum(grad * v_grad)',
            fontsize=14, fontfamily='monospace')
    ax.text(1.0, 1.4, 'hv = torch.autograd.grad(grad_dot_v, x_grad, retain_graph=True)[0]',
            fontsize=14, fontfamily='monospace')

    # Advantages
    ax.text(8, 2.5, 'Advantages:', fontsize=16, fontweight='bold')
    ax.text(8, 2, '• Memory complexity: O(n) instead of O(n²)', fontsize=16)
    ax.text(8, 1.7, '• Computational complexity: O(n) instead of O(n²)', fontsize=16)
    ax.text(8, 1.4, '• No need to store complete Hessian matrix', fontsize=16)
    ax.text(8, 1.1, '• Suitable for large-scale optimization problems', fontsize=16)
