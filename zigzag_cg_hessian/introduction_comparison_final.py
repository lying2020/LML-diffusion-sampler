#!/usr/bin/env python3
"""
Introduction Comparison: DDIM, PNDM (Final Version)
...
class IntroductionComparison:
    """Comparison of DDIM, PNDM for introduction section"""
    ...
    def plot_zigzag_comparison(self, results_dict, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        """Plot 2x1 zigzag comparison for introduction"""
        os.makedirs(save_dir, exist_ok=True)

        # Create figure with 2 subplots in one column
        fig, axes = plt.subplots(2, 1, figsize=(8, 10))
        fig.suptitle('Zigzag Phenomenon Comparison: DDIM vs PNDM\n(PCA1 vs PCA2 - 20 Steps)',
                     fontsize=16, fontweight='bold')

        # Define colors for each method
        colors = ['#E74C3C', '#9B59B6']  # Red, Purple
        method_names = ['DDIM', 'PNDM']
    ...
    def plot_anisotropy_transition(self, results_dict, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        """Plot 2x1 anisotropy transition comparison for introduction"""
        os.makedirs(save_dir, exist_ok=True)

        # Create figure with 2 subplots in one column
        fig, axes = plt.subplots(2, 1, figsize=(8, 10))
        fig.suptitle('Anisotropy Transition: DDIM vs PNDM\n(Isotropic to Anisotropic - 20 Steps)',
                     fontsize=16, fontweight='bold')
    ...
    def generate_introduction_report(self, results_dict, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        ...
        with open(report_path, 'w') as f:
            f.write("Introduction Comparison: DDIM vs PNDM\n")
            f.write("="*70 + "\n\n")
...
def main():
    ...
    print("🚀 Introduction Comparison: DDIM vs PNDM (Final Version)")
    print("="*70)
