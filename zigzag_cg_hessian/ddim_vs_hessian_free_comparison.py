#!/usr/bin/env python3
"""
DDIM vs Hessian-Free Method Zigzag Comparison
...
class DDIMvsHessianFreeAnalyzer:
    ...
    def load_cifar10_samples(self):
        ...
        print(f"\n📊 Loading {self.n_samples} CIFAR-10 samples for PCA...")
        ...
        print(f"✓ Loaded {X.shape[0]} CIFAR-10 samples (1024-dimensional)")
        return X
    ...
    def plot_ddim_vs_hessian_free(self, results_dict, save_dir='./zigzag_cg_hessian'):
    ...
    def generate_sample_statistics_report(self, results_dict, save_dir='./zigzag_cg_hessian'):
        """Generate detailed sample statistics report"""
        os.makedirs(save_dir, exist_ok=True)
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        report_path = os.path.join(save_dir, f'sample_statistics_report_{timestamp}.txt')
...
def main():
    ...
    print("🚀 DDIM vs Hessian-Free Method: Zigzag Analysis")
    print("="*60)
    ...
    print(f"\n📊 Performing PCA on CIFAR-10 data...")
    ...
    try:
        for method in methods:
            ...
            # Analyze zigzag patterns
            results = analyzer.analyze_zigzag_patterns(trajectories, method, pca_model)
