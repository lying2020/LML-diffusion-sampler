#!/usr/bin/env python3
"""
Introduction Comparison: DDIM, UniPC, PNDM (Simplified)
...
# Import schedulers
from diffusers import DDPMPipeline, DDIMScheduler, PNDMScheduler, UniPCMultistepScheduler
...
class IntroductionComparison:
    """Comparison of DDIM, UniPC, PNDM for introduction section"""

    def __init__(self, n_samples=5000, num_inference_steps=20, num_trajectories=50):
        self.n_samples = n_samples
        self.num_inference_steps = num_inference_steps
        self.num_trajectories = num_trajectories
        self.methods = ['ddim', 'unipc', 'pndm']
    ...
    def plot_zigzag_comparison(self, results_dict, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        """Plot 3x1 zigzag comparison for introduction"""
        os.makedirs(save_dir, exist_ok=True)

        # Create figure with 3 subplots in one column
        fig, axes = plt.subplots(3, 1, figsize=(8, 12))
        fig.suptitle('Zigzag Phenomenon Comparison: DDIM, UniPC, PNDM\n(PCA1 vs PCA2 - 20 Steps)',
                     fontsize=16, fontweight='bold')

        # Define colors for each method
        colors = ['#E74C3C', '#F39C12', '#9B59B6']  # Red, Orange, Purple
        method_names = ['DDIM', 'UniPC', 'PNDM']
        ...
        for i, (method, result) in enumerate(results_dict.items()):
            ...
            sample_indices = np.arange(0, len(xt_pca), max(1, len(xt_pca)//20))
    ...
    def plot_anisotropy_transition(self, results_dict, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        """Plot 3x1 anisotropy transition comparison for introduction"""
        os.makedirs(save_dir, exist_ok=True)

        # Create figure with 3 subplots in one column
        fig, axes = plt.subplots(3, 1, figsize=(8, 12))
        fig.suptitle('Anisotropy Transition: DDIM, UniPC, PNDM\n(Isotropic to Anisotropic - 20 Steps)',
                     fontsize=16, fontweight='bold')
    ...
    def generate_introduction_report(self, results_dict, save_dir=os.path.join(project.output_dir, 'zigzag_cg_hessian')):
        ...
        with open(report_path, 'w') as f:
            f.write("Introduction Comparison: DDIM, UniPC, PNDM\n")
            f.write("="*70 + "\n\n")
...
def main():
    ...
    print("🚀 Introduction Comparison: DDIM, UniPC, PNDM (Simplified)")
    print("="*70)
    print("Comparing zigzag phenomena and anisotropy transition for introduction section")
    print("="*70)

    # Initialize analyzer
    analyzer = IntroductionComparison(n_samples=5000, num_inference_steps=20, num_trajectories=50)
