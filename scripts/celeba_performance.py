#!/usr/bin/env python3
"""
CelebA Performance Analyzer
整理CelebA生成结果中的性能数据，包括不同步数和方法下的生成耗时统计
"""
...
class CelebAPerformanceAnalyzer:
    ...
    def parse_json_file(self, file_path):
        ...
        try:
            ...
            if step is None or method is None:
                print(f"Warning: Could not extract step or method from {file_path}")
                return None
    ...
    def analyze_performance(self):
        ...
        print(f"成功解析 {len(self.performance_data)} 个文件")
    ...
    def generate_summary_table(self):
        ...
        for step in sorted(self.df['step'].unique()):
            ...
            for method in sorted(step_data['method'].unique()):
                ...
                print(f"  {method:12}: {avg_time:.4f}s/img, {avg_ips:.2f} img/s, {total_images} images")
    ...
    def generate_detailed_report(self, save_dir="output/celeba"):
        ...
        if not self.df.empty:
            ...
            print(f"📊 时间对比表已保存到: {time_csv}")
            print(f"📊 速度对比表已保存到: {ips_csv}")
    ...
    def generate_latex_table(self, save_dir, timestamp):
        ...
        print(f"📝 LaTeX时间对比表已保存到: {time_latex_path}")
        print(f"📝 LaTeX速度对比表已保存到: {ips_latex_path}")
...
\\end{{table*}}"""
    ...
    def print_statistics(self):
        ...
        print(f"步数范围: {self.df['step'].min()} - {self.df['step'].max()}")
        print(f"方法数: {len(self.df['method'].unique())}")
        print(f"总生成图片数: {self.df['total_images'].sum()}")
        print(f"总耗时: {self.df['total_time'].sum():.2f} 秒")
        ...
        print(f"🐌 最慢方法: {slowest_method['method']} (步数{slowest_method['step']}) - {slowest_method['images_per_second']:.2f} img/s")

def main():
    parser = argparse.ArgumentParser(description='CelebA Performance Analyzer')
    parser.add_argument('--output_dir', default='output/celeba', help='Output directory path')
    parser.add_argument('--save_dir', default='output/celeba', help='Save directory for reports')
    ...
    # 创建分析器
    analyzer = CelebAPerformanceAnalyzer(args.output_dir)
    ...
    try:
        ...
        # 生成详细报告
        analyzer.generate_detailed_report(args.save_dir)
